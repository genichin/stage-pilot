...
- 연계 문서 현황:
	- Planning 점수 파일: {{PLANNING_SCORE_FILE:미생성 또는 경로}}
...
> **판정이 `완전 중복`인 경우**: 이하 섹션 작성을 중단하고 `# 9. 파일 처리 결과`에 중복 판정 사유와 참조 문서 경로를 기록한 뒤 문서를 닫는다. Planning으로 넘기지 않는다.
...
- 코드 영향:
	- {{CODE_IMPACT_1:예. docs/sdlc/index.md 갱신 로직}}
...
## Freeze 플래그
- Handoff Decision: {{FREEZE_HANDOFF_DECISION:Planning 진행 가능|보류}}
- Handoff Rationale: {{FREEZE_HANDOFF_RATIONALE:진행 가능 또는 보류 판단 근거 1-2줄}}
- Ready for Planning: {{READY_FOR_PLANNING:false}}
