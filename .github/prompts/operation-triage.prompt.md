...
7. 새 SDLC 아이템 생성 필요 여부 결정 규칙:
   - 구현 문제이고 관련 문서가 confirmed 또는 이미 Release/Operations까지 진행된 상태라면 새 SDLC 아이템 생성 필요
   - 구현 문제이지만 관련 문서가 전부 draft 상태이고 기존 범위 안에서 흡수 가능한 수정이면 새 SDLC 아이템 생성 불필요
   - 설계/요구사항 누락이면 새 SDLC 아이템 생성 필요
   - 신규 범위이면 새 SDLC 아이템 생성 필요
   - 판단 보류이면 새 SDLC 아이템 생성 여부도 보류로 둔다.
   - 운영 절차/모니터링/런북/커뮤니케이션만 보완하면 되는 경우 새 SDLC 아이템 생성 불필요
   - Hotfix가 필요한 경우에도 최소 요구사항 정의, 최소 검증, 롤백 계획, Postmortem 환류가 필요함을 명시한다.
8. 새 Discovery 시작 여부 기준표를 아래 우선순위대로 적용한다.
   - 구현 문제 + confirmed 이후 문서 변경 필요: 새 Discovery 필요
   - 구현 문제 + draft 문서에 흡수 가능: 새 Discovery 불필요
   - 설계/요구사항 누락: 새 Discovery 필요
   - 신규 범위: 새 Discovery 필요
   - 운영 보완만 필요: 새 Discovery 불필요
   - 서비스 중단/데이터 손실/보안 영향으로 Hotfix 우선: 즉시 복구 후 후속 Discovery 필요
   - 근거 부족/상충: 판단 보류, 추가 정보 수집 후 재실행
9. 근거 제시 규칙:
   - 분류와 결정에는 반드시 문서 경로 또는 본문 근거를 붙인다.
   - 근거가 약하면 판단 보류로 내리고, 부족한 정보를 명시한다.
10. 이 프롬프트는 문서를 자동 수정하거나 생성하지 않는다.
