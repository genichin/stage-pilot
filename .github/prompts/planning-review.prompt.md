---
description: "Use when: /planning-review to re-review existing planning documents and update them in place from Discovery IDs"
...
사용자가 지정한 Discovery 문서 ID 목록을 기준으로 Planning 문서를 재검토하고, 필요한 보정을 반영해 저장한다.

반드시 아래 규칙을 지켜라.
1. 출력은 한국어로 작성한다.
2. 입력은 쉼표(,) 기준으로 분리해 Discovery ID 목록으로 처리한다.
3. 입력 정규화 규칙:
   - 공백 제거
   - 소문자 정규화(dcy-001 형식)
   - 중복 ID 제거
...
5. Planning 파일 검색 규칙:
   - 1차: dcy-001 -> docs/sdlc/planning/pln-001_*.planning.md 패턴 검색
   - 2차: 1차 미발견 시 docs/sdlc/planning/*.planning.md에서 `입력 Discovery:` 줄에 dcy-001 포함 문서 검색
...
8. 문서 재검토 항목:
   ...
   - `# 4. WSJF 점수표`에 BV/TC/RR-OE/Job Size/WSJF/Priority가 존재하는지 검증
   - WSJF 우선순위가 selected_score(WSJF) 내림차순과 일치하는지 검증
...
2. 각 ID에 대해 Planning 문서 탐색(1차/2차 규칙 적용)
3. 발견 문서 내용 읽기
